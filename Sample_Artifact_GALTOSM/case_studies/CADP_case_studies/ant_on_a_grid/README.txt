...
This folder contains the the model of the ant on a grid problem written in LNT and saved as '.lnt'.

For each filename 'ant$GRIDSIZE.lnt' the $GRIDSIZE represents the size of the grid.
...
3)RUN THE COMMAND: lnt.open ant8.lnt generator ant8.bcg (to get the time put 'time' in front of the lnt.open)
	You will get a '.bcg' file with the name 'ant8' in the same location.
	
4)RUN THE COMMAND: bcg_io ant8.bcg ant8.aut
	You will get a '.aut' file in the same location. which is a human readable file.
	
5)RUN THE COMMAND: sed -i 's/ !/; prob /g' ant8.aut
	All the '!' mark will get replaced by '; prob' in the '.aut file

Now this 'ant8.aut' is an ADTMC which will be provided as input to GALTOSM's model translator tool. (which are also provided in $BASE_DIRECTORY/Artifact_GALTOSM/GALTOSM_tool/test_cases)

step 6 and 7 is for model checking in CADP
6)RUN THE COMMAND: bcg_io ant8.aut ant8.bcg
	This will replace the previous '.bcg' file which was present previously with the '.bcg' of the modified '.aut' file.
7)RUN THE COMMAND: bcg_open ant8.bcg evaluator5 ant_property.mcl
...
MCL of APCTL:
______________
We have expressed our requirement using the APCTL formula in the file 'ant_property_APCTL.txt'. Since CADP model checking requires '.mcl' file for the specification. We have written MCL5 equivalent formula of the APCTL formula in the 'ant_property.mcl' file.
...
STEPS:
		...
		5) RUN THE COMMAND 
				...
				./storm --explicit $BASE_DIRECTORY/Artifact_GALTOSM/GALTOSM_tool/test_cases/adtmc_to_sdtmc/ant8.tra $BASE_DIRECTORY/Artifact_GALTOSM/GALTOSM_tool/test_cases/adtmc_to_sdtmc/ant8.lab --prop "(P=?[((\"bot\"&(true))|(!(\"bot\")&(true)))U((!(\"bot\")&(\"LIVE\"))&P>=1[X(\"bot\"&(true))])])"
