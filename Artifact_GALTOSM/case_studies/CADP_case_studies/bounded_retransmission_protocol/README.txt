...
This folder contains the the model of the Bounded retransmission protocol problem written in LNT and saved as '.lnt'.

For each filename 'brp_$RETRY_$FRAME.lnt' the $RETRY represents the number of retries and $FRAME represents the number of frames needs to be transferred.
...
3)RUN THE COMMAND: lnt.open brp_10_101.lnt generator brp_10_101.bcg (to get the time put 'time' in front of the lnt.open)
	You will get a '.bcg' file with the name 'brp_10_101' in the same location.
	
4)RUN THE COMMAND: bcg_io brp_10_101.bcg brp_10_101.aut
	You will get a '.aut' file in the same location. which is a human readable file.
	
5)RUN THE COMMAND: sed -i 's/ !/; prob /g' brp_10_101.aut
	All the '!' mark will get replaced by '; prob' in the '.aut file

Now this 'brp_10_101.aut' is an ADTMC which will be provided as input to GALTOSM's model translator tool. (which are also provided in $BASE_DIRECTORY/Artifact_GALTOSM/GALTOSM_tool/test_cases)

step 6 and 7 is for model checking in CADP
6)RUN THE COMMAND: bcg_io brp_10_101.aut brp_10_101.bcg
	This will replace the previous '.bcg' file which was present previously with the '.bcg' of the modified '.aut' file.
7)RUN THE COMMAND: bcg_open brp_10_101.bcg evaluator5 property_brp.mcl
...
MCL of APCTL:
______________
We have expressed our requirement using the APCTL formula in the file 'property_brp_APCTL.txt'. Since CADP model checking requires '.mcl' file for the specification. We have written MCL5 equivalent formula of the APCTL formula in the 'property_brp.mcl' file.
...
STEPS:
		...
		5) RUN THE COMMAND 
				...
				./storm --explicit $BASE_DIRECTORY/Artifact_GALTOSM/GALTOSM_tool/test_cases/adtmc_to_sdtmc/brp_10_101.tra $BASE_DIRECTORY/Artifact_GALTOSM/GALTOSM_tool/test_cases/adtmc_to_sdtmc/brp_10_101.lab --prop "(P=?[((\"bot\"&(true))|(!(\"bot\")&(true)))U((!(\"bot\")&(\"LIVE\"))&P>=1[X(\"bot\"&(true))])])"
