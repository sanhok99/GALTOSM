...
This folder contains the the model of the gambling problem written in LNT and saved as '.lnt'.

For each filename 'gamble_$GAMES_$TARGET.lnt' the $GAMES represents the number of the player plays and $TARGET represents the amount the player targets to achieve.
...
3)RUN THE COMMAND: lnt.open gamble_3_105.lnt generator gamble_3_105.bcg (to get the time put 'time' in front of the lnt.open)
	You will get a '.bcg' file with the name 'gamble_3_105' in the same location.
	
4)RUN THE COMMAND: bcg_io gamble_3_105.bcg gamble_3_105.aut
	You will get a '.aut' file in the same location. which is a human readable file.
	
5)RUN THE COMMAND: sed -i 's/ !/; prob /g' gamble_3_105.aut
	All the '!' mark will get replaced by '; prob' in the '.aut file

Now this 'gamble_3_105.aut' is an ADTMC which will be provided as input to GALTOSM's model translator tool. (which are also provided in $BASE_DIRECTORY/Artifact_GALTOSM/GALTOSM_tool/test_cases)

step 6 and 7 is for model checking in CADP
6)RUN THE COMMAND: bcg_io gamble_3_105.aut gamble_3_105.bcg
	This will replace the previous '.bcg' file which was present previously with the '.bcg' of the modified '.aut' file.
7)RUN THE COMMAND: bcg_open gamble_3_105.bcg evaluator5 property_gamble.mcl
...
MCL of APCTL:
______________
We have expressed our requirement using the APCTL formula in the file 'property_gamble_APCTL.txt'. Since CADP model checking requires '.mcl' file for the specification. We have written MCL5 equivalent formula of the APCTL formula in the 'property_gamble.mcl' file.
...
STEPS:
		...
		5) RUN THE COMMAND 
				...
				./storm --explicit $BASE_DIRECTORY/Artifact_GALTOSM/GALTOSM_tool/test_cases/adtmc_to_sdtmc/gamble_3_105.tra $BASE_DIRECTORY/Artifact_GALTOSM/GALTOSM_tool/test_cases/adtmc_to_sdtmc/gamble_3_105.lab --prop "(P=?[(("bot"&(true))|(!("bot")&(true)))U((!("bot")&("HOME" | "DRUNK"))&P>=1[X("bot"&(true))])])"
